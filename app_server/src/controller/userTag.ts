...
import { UserTagService } from '../service/userTag';
...
export class UserTagController extends BaseController {
  @Inject()
  ctx: Context;
  ...
  async findAll(): Promise<any> {
    const result = await this.userTagService.findAll();
  ...
  async findById(@Param() id: number): Promise<any> {
    const result = await this.userTagService.findById(id);
    return this.SUCCESS_200().setData(result);
  }
  ...
  async create(@Body(ALL) body: any): Promise<any> {
    const result = await this.userTagService.create(body, this.ctx.user);
  ...
  async update(@Body() id: number, @Body(ALL) body: any): Promise<any> {
    const result = await this.userTagService.updateById(
      id,
      body,
      this.ctx.user
    );
  ...
  async delete(@Param() id: number): Promise<any> {
    const result = await this.userTagService.logicDeleteById(id, this.ctx.user);
