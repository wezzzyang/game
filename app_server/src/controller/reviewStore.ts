...
import { ReviewStoreService } from '../service/reviewStore';
import { BaseController } from '../lib/baseController';
import { CreateApiDoc } from '@midwayjs/swagger';
import { ILogger } from '@midwayjs/logger';

@Provide()
@Controller('/api/reviewStore', {
  tagName: '名称：reviewStore',
  description: 'DESC',
})
export class ReviewStoreController extends BaseController {
  @Inject()
  ctx: Context;
  ...
  async findAll(): Promise<any> {
    const result = await this.reviewStoreService.findAll();
  ...
  async findById(@Param() id: number): Promise<any> {
    const result = await this.reviewStoreService.findById(id);
    return this.SUCCESS_200().setData(result);
  }
  ...
  async create(@Body(ALL) body: any): Promise<any> {
    const result = await this.reviewStoreService.create(body, this.ctx.user);
  ...
  async update(@Body() id: number, @Body(ALL) body: any): Promise<any> {
    const result = await this.reviewStoreService.updateById(
      id,
      body,
      this.ctx.user
    );
  ...
  async delete(@Param() id: number): Promise<any> {
    const result = await this.reviewStoreService.logicDeleteById(id, this.ctx.user);
