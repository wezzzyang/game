...
export class TopicController extends BaseController {
  ...
  async create(@Body(ALL) body: any): Promise<any> {
    const result = await this.topicService.create(body, this.ctx.user);
  ...
  async update(@Body() id: number, @Body(ALL) body: any): Promise<any> {
    const result = await this.topicService.updateById(
      id,
      body,
      this.ctx.user
    );
  ...
  async delete(@Param() id: number): Promise<any> {
    const result = await this.topicService.logicDeleteById(id, this.ctx.user);
