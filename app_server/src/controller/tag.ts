...
export class TagController extends BaseController {
  ...
  @(CreateApiDoc()
    .summary('查找全部rank =1 rank = 3数据')
    .description('Get请求，查找全部数据!')
    .build())
  @Get('/getAlltag')
  async getAlltag(): Promise<any> {
    const result = await this.tagService.getTagAll();
  ...
  async create(@Body(ALL) body: any): Promise<any> {
    const result = await this.tagService.create(body, this.ctx.user);
  ...
  async update(@Body() id: number, @Body(ALL) body: any): Promise<any> {
    const result = await this.tagService.updateById(id, body, this.ctx.user);
  ...
  async delete(@Param() id: number): Promise<any> {
    const result = await this.tagService.logicDeleteById(id, this.ctx.user);
